...
//		}
//		if(hasName = true) System.out.println(" 이름 등록 됨");
//		else System.out.println("등록 되지 않은 이름");
		
//		// 배열->문자열
//		String temp = String.join("," names);
//		// 박문수,김정호,서경림,김명준,이정훈
//		boolean hasName = temp.
		
		
		
		// 학과명, 조교, 전화, 위치
		String[][] dept = new String[4][];
		dept[0] = new String[] {"컴퓨터공학과","정보과학과","정보통신학과","정보산업공학과","전자공학과","문헌정보학과","전기공학과"};
		...
		int idx=-1;		
			...
			if(idx == -1) // 만약 학과가 존재 하지 않으면
			{
				System.out.println("등록 되지 않은 학과 입니다.");
				return; // return은 메서드를 종료시킨다. main메서드 종료 되면 프로그램 끝
			}
...
//		배열의주소를 가지고 있는 참조변수는 참조변수이름[인덱스] 방식으로 사용

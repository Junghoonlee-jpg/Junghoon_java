...
public class Member {
	...
	String name = "박문수"; // 이름 (인스턴스 변수)
	int age; // 나이
	String addr; // 주소
	String dept; // 학과
	
	static{ // 클래스 초기화 블럭  - 클래스 변수만 초기화가능 
...
/*
 	...
 	메서드의 종류
 	   ...
 	   클래스 메서으ㅔ서 인스턴스 메서드 사용불가
 	   
 	   그래서 정리하자면!!
 	   
 	   클래스변수(static) 또는 클래스메서드(static)은 프로그램 실행시 생성
 	   
 	   인스턴스변수 또는 인스턴스 메서드는 객체 생성시 생성
 	   
 	   프로그램실행 -> 클래스정의부 메모리 로드 -> 클래스변수 또는 클래스메서드 생성 -> main 메서드 실행 ->
 	   new Member() 생성자메서드 호출 -> 객체생성 -> 인스턴스 변수또는 인스턴스 메서드 생성
 	   
 	   그래서 초기화 순서는!!
 	   
 	   클래스변수 명시적초기화 -> 클래스 초기화블럭 -> 인스턴스변수 명시적 초기화 -> 인스턴스 초기화블럭 ->
 	   생성자 메서드
